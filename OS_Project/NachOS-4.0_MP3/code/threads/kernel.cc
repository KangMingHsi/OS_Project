...
Kernel::Initialize()
{
    ...
    currentThread->setExecPriority(200);
...
void
Kernel::ConsolePrintInt(int number)
{
...
void Kernel::ExecAll()
{
	for (int i=1;i<=execfileNum;i++) {
		int a;
		if(priorityEnabled)
		    a = Exec(execfile[i], execfilePriority[i]);
		else
		    a = Exec(execfile[i]);
	}
...
{
	return fileSystem->mOpen(name);
}

int Kernel::Write(char *buffer, int size, OpenFileId id)
{
	
	return fileSystem->Write(buffer, size * sizeof(char), id);
}

int Kernel::Read(char *buffer, int size, OpenFileId id)
{
	return fileSystem->Read(buffer, size * sizeof(char), id);
}

int Kernel::Close(OpenFileId id)
{
	return fileSystem->mClose(id);
}
